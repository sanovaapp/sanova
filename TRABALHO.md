...
- [x] Retenção automática de dados — `alert_events` com mais de 180 dias
      apaga de verdade; conta inativa há 24 meses só conta, não apaga
      (irreversível demais pra ser efeito colateral de cron). Conta apagada
      já respeitava os 30 dias da política. Cron diário 03:00 BRT,
      `retencao.yml` + `worker/src/retencao.js`. *(PR #254, 07/08)*
- [x] Ligar a Fase 2 (alertas ao profissional) — `FASE2_ALERTAS_ATIVA = true`
      em `pro.html`. Padrão mais conservador que já existia no código: os 5
      vermelhos nascem em `imediato`, os 5 amarelos em `off`. *(PR #254, 07/08)*
